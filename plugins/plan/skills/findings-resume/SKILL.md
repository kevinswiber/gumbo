...
1. **Identify the target plan:**
   ...
   - If no number is provided, scan for plans with findings:
     ...
     - If multiple plans have findings, list them and ask the user to choose:
       ...
       1. `.thoughts/plans/0018-lr-rl-remaining-fixes/` - 4 findings (in_progress)
       2. `.thoughts/plans/0020-edge-routing/` - 2 findings (in_progress)
...
3. **Present findings summary to the user:**
   ...
   **Proposed actions:**
   - **Issues (N):** Findings 1, 4, 5 → new issue set `.thoughts/issues/NNNN-description/`
   - **Research (N):** Finding 2 → update `.thoughts/research/NNNN-topic/`; Finding 6 → new research
...
5. **Create issues** from findings that warrant them:
   ...
   - Find the next issue set number by checking `.thoughts/issues/` for the highest `NNNN-*` prefix
   - Create `.thoughts/issues/NNNN-kebab-description/` directory
   ...
   - Create `.thoughts/issues/issues/` subdirectory with individual issue files
   - Each issue file includes:
     ...
     - Link back to the source finding: `.thoughts/plans/NNNN-name/findings/finding-name.md`
   - Follow the format conventions in `.thoughts/issues/CLAUDE.md`
...
6. **Route research findings** to the appropriate location:
   ...
   **Update existing research** if the finding relates to an active or archived research topic:
   - Scan `.thoughts/research/NNNN-*/research-plan.md` and `.thoughts/research/archive/NNNN-*/research-plan.md` for topic matches
   - If a matching research directory exists and is active (`.thoughts/research/NNNN-*/`):
     - Add a new findings file: `.thoughts/research/NNNN-topic/from-plan-MMMM-description.md`
   ...
   **Create new research** if the finding opens a new topic not covered by existing research:
   - Find the next research number by checking both `.thoughts/research/` and `.thoughts/research/archive/`
   - Create `.thoughts/research/NNNN-topic-name/` with a research plan
   - Follow `.thoughts/research/CLAUDE.md` conventions
...
7. **Report results:**
   ...
   **Issues created:** `.thoughts/issues/NNNN-description/`
   - Issue 01: [title] (High)
   - Issue 02: [title] (Medium)

   **Research updated:** `.thoughts/research/NNNN-topic/`
   - Added: `from-plan-MMMM-description.md`

   **Research created:** `.thoughts/research/NNNN-new-topic/`
   - Status: PLANNED — run `/research:resume` to investigate

   **No action:** 2 findings (informational)

   **Source:** `.thoughts/plans/MMMM-name/findings/` (N findings triaged)
   ```
...
**Plan:** `.thoughts/plans/0020-edge-routing/`
**Findings:** 5 total

| # | Finding | Type | Task | Proposed Action |
|---|---------|------|------|-----------------|
| 1 | Diamond nodes need special attachment logic | discovery | 2.1 | Research update → `.thoughts/research/0013-visual-comparison-fixes/` |
...
**Proposed actions:**
- **Issues (3):** Findings 2, 3, 5 → new issue set `.thoughts/issues/0003-edge-routing-findings/`
- **Research (1):** Finding 1 → update `.thoughts/research/0013-visual-comparison-fixes/`
